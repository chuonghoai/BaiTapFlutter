...
class _RegisterViewState extends State<RegisterView> {
  ...
  Future<void> _handleSendOtp() async {
    ...
    await _controller.handleSendOtp(
        context, _emailCtrl.text.trim(), _usernameCtrl.text.trim(), _passCtrl.text);
    if (mounted) setState(() { _loading = false; _otpSent = true; });
  }

  Future<void> _handleVerifyOtp() async {
    setState(() => _loading = true);
    await _controller.handleVerifyOtp(context, _emailCtrl.text.trim(), _otpCtrl.text.trim());
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              const Text('Nhập thông tin để đăng ký, OTP sẽ được gửi qua email.',
                  style: TextStyle(fontSize: 13, color: Colors.grey)),
              const SizedBox(height: 28),

              _buildTextField(controller: _emailCtrl,
                  label: 'Email', icon: Icons.email_outlined,
                  enabled: !_otpSent),
              const SizedBox(height: 14),
              _buildTextField(controller: _usernameCtrl,
                  label: 'Tên đăng nhập', icon: Icons.person_outline,
                  enabled: !_otpSent),
              const SizedBox(height: 14),
              _buildTextField(controller: _passCtrl,
                  label: 'Mật khẩu', icon: Icons.lock_outline,
                  obscure: _obscure, enabled: !_otpSent,
                  suffix: IconButton(
              ...
              if (_otpSent) ...[
                const SizedBox(height: 14),
                _buildTextField(controller: _otpCtrl,
                    label: 'Nhập mã OTP', icon: Icons.verified_outlined),
              ],
